...
if typeof(Drawing) == "table" then
    if typeof(Drawing.new) == "function" and typeof(Drawing.clear) == "function" and typeof(Drawing.Fonts) == "table" then
        local test_types = {
            "Line",
            "Square",
            "Text",
        ...
        local success = pcall(function()
            for _, test in pairs(test_types) do
                local obj = Drawing.new(test)
                
                if typeof(obj) ~= "table" or typeof(obj.Remove) ~= "function" then
                    error("Invalid object returned from Drawing.new(" .. test .. ")")
                end
                ...
                if test == "Line" then
                    ...
                    obj.Thickness = 2
                    obj.Visible = true
                elseif test == "Square" then
                    obj.Position = Vector2.new(0,0)
                    obj.Size = Vector2.new(1,1)
                    obj.Color = Color3.new(0,1,0)
                    obj.Filled = true
                    obj.Thickness = 1
                    obj.Visible = true
                elseif test == "Text" then
                    obj.Text = "test"
                    obj.Position = Vector2.new(0, 0)
                    obj.Size = 14
                    obj.Font = 1
                    ...
                    obj.Outline = true
                    obj.OutlineColor = Color3.new(1, 1, 1)
                    obj.Visible = true
                elseif test == "Triangle" then
...
function parseColor(self, color, isOutline)
    if color == "Team Color" or (self.interface.sharedSettings.useTeamColor and isOutline) then
...
function EspObject:Render()
	...
	if typeof(hidden.arrow) == "table" then
